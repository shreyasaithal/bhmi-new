...
<div class="header-item cart header__item header__item__cart--<?php echo esc_attr( av5_get_option( 'header-cart-type' ) ); ?>">
	...
	<div id="av5_woocommerce_mini_cart_drop_down" class="av5_woocommerce_mini_cart_drop <?php
	if ( $promo_text ) {
	...
	?>" style="display: none;">
			<?php
			if ( $promo_text ) {
				echo sprintf( '<div class="mini-cart-promo-text">%s</div>', $promo_text ); // WPCS: xss ok.
			}
			?>
			<?php if ( av5_get_option( 'header-cart-type' ) == 'drop-down' ) { ?>
