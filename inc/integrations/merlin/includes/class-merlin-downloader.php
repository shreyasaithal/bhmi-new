...
class Merlin_Downloader {
	 ...
	 */
	public function esc_html__construct( $download_directory_path = '' ) {
	...
	public function download_file( $url, $filename ) {
		...
		$saved_file = file_put_contents( $this->download_directory_path . $filename, $content ); // @codingStandardsIgnoreLine WordPress.VIP.FileSystemWritesDisallow.FileWriteDetected
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new \WP_Error(
				'missing_url', esc_html__( 'Missing URL for downloading a file!', 'pt-ocdi' )
			);
		}

		// Get file content from the server.
		$response = wp_remote_get(
			$url, array( 'timeout' => apply_filters( 'merlin_timeout_for_downloading_import_file', 20 ) )
		);
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error', sprintf(
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'pt-ocdi' ), '<strong>', $url, '</strong>', '<br>', $response_error['error_code'], $response_error['error_message']
				)
			);
		}
