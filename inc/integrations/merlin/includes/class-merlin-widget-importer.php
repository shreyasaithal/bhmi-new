...
class Merlin_Widget_Importer {
	...
	private static function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new \WP_Error(
				'widget_import_file_not_found', esc_html__( 'Error: Widget import file could not be found.', 'merlin-wp' )
			);
		}
	...
	private static function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new \WP_Error(
				'corrupted_widget_import_data', esc_html__( 'Error: Widget import data could not be read. Please try a different file.', 'merlin-wp' )
			);
		}
	 ...
	 */
	public static function unset_default_widgets() {
	...
	private static function format_results_for_log( $results ) {
		...
		foreach ( $results as $sidebar ) {
			echo esc_html( $sidebar['name'] ) . ' : ' . esc_html( $sidebar['message'] ) . PHP_EOL . PHP_EOL; // WPCS: xss ok.
			// Loop widgets.
			foreach ( $sidebar['widgets'] as $widget ) {
				echo esc_html( $widget['name'] ) . ' - ' . esc_html( $widget['title'] ) . ' - ' . esc_html( $widget['message'] ) . PHP_EOL; // WPCS: xss ok.
			}
			echo PHP_EOL; // WPCS: xss ok.
		}
	}
