...
class Merlin_Customizer_Importer {
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				'missing_cutomizer_import_file', sprintf(
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'pt-ocdi' ), $import_file_path
				)
			);
		}

		// Get the upload data.
		$raw = file_get_contents( $import_file_path ); // @codingStandardsIgnoreLine WordPress.VIP.RestrictedFunctions.file_get_contents
		...
		$data = unserialize( $raw ); // @codingStandardsIgnoreLine WordPress.VIP.RestrictedFunctions.serialize

		// Data checks.
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				'customizer_import_data_error', esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'pt-ocdi' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme', esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'pt-ocdi' )
			);
		}
	...
	private static function customizer_sideload_image( $file ) {
		...
		if ( ! empty( $file ) ) {
			...
			if ( is_wp_error( $id ) ) {
				unlink( $file_array['tmp_name'] ); // @codingStandardsIgnoreLine WordPress.VIP.FileSystemWritesDisallow.FileWriteDetected
				return $id;
			}
