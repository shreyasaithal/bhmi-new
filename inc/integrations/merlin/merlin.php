...
class Merlin {
	...
	function required_classes() {
		...
		require_once get_parent_theme_file_path( $this->directory . '/merlin/vendor/autoload.php' );
	...
	public function admin_page() {
		...
		if ( empty( $_GET['page'] ) || $this->merlin_url !== $_GET['page'] ) { // WPCS: input var ok.
			return;
		}
		...
		$this->step	 = isset( $_GET['step'] ) ? sanitize_key( $_GET['step'] ) : current( array_keys( $this->steps ) ); // WPCS: input var ok.
		...
		<div class="merlin__wrapper">
			...
			<div class="merlin__content merlin__content--<?php echo esc_attr( strtolower( $this->steps[ $this->step ]['name'] ) ); ?>">
				...
				if ( ! empty( $_REQUEST['save_step'] ) && isset( $this->steps[ $this->step ]['handler'] ) ) { // WPCS: input var ok.
	...
	protected function header() {
		...
		<html xmlns="http://www.w3.org/1999/xhtml" <?php language_attributes(); ?>>
			...
			<body <?php body_class( 'merlin__body merlin__body--' . $current_step ); ?>>
				<?php
	}
		...
		<footer class="merlin__content__footer">
			...
			<?php if ( ! $is_child_theme ) : ?>
				...
				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="merlin__button merlin__button--next button-next" data-callback="install_child">
					<span class="merlin__button--loading__text"><?php echo esc_html( $install ); ?></span><?php echo  $this->loading_spinner(); // WPCS: xss ok. ?>
				</a>
	...
	protected function plugins() {
		...
		$method	 = '';
		$fields	 = array_keys( $_POST ); // WPCS: input var ok. CSRF ok.
					...
					<?php foreach ( $plugins['all'] as $slug => $plugin ) : ?>
						...
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<span>
								...
								echo implode( esc_html__( 'and', 'merlin-wp' ), $keys ); // WPCS: xss ok.
								?>
	...
	protected function content() {
		...
		<form action="" method="post">
			...
			<ul class="merlin__drawer merlin__drawer--import-content js-merlin-drawer-import-content">
				<?php echo $this->get_import_steps_html( $import_info ); // WPCS: xss ok.  ?>
			</ul>
			...
			<footer class="merlin__content__footer">
				...
				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="merlin__button merlin__button--next button-next" data-callback="install_content">
					<span class="merlin__button--loading__text"><?php echo esc_html( $import ); ?></span><?php echo $this->loading_spinner(); // WPCS: xss ok.   ?>
				</a>
	...
	protected function ready() {
		...
		// Links.
		$link_1	 = isset( $strings['ready-link-1'] ) ? $strings['ready-link-1'] : '';
		$link_2	 = isset( $strings['ready-link-2'] ) ? $strings['ready-link-2'] : '';
		$link_3	 = isset( $strings['ready-link-3'] ) ? $strings['ready-link-3'] : '';
		...
		<div class="merlin__content--transition">
			...
			<p><?php wp_kses( printf( $paragraph, $author ), $allowed_html_array ); // WPCS: xss ok.   ?></p>
...
* Description: Create unforgettable shopping experience for your customers with our powerful eCommerce WordPress theme - {$parent}.
* Author: {$author}
...
-------------------------------------------- */
	...
	function _ajax_plugins() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) { // WPCS: input var ok.
			exit( 0 );
		}
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) { // WPCS: input var ok.
				$json = array(
					'url'			 => $tgmpa_url,
					'plugin'		 => array( $slug ),
		...
		if ( $json ) {
			$json['hash'] = md5( serialize( $json ) ); // @codingStandardsIgnoreLine WordPress.VIP.RestrictedFunctions.serialize
			wp_send_json( $json );
		} else {
	...
	function _ajax_content() {
		...
		$selected_import = intval( @ $_POST['selected_index'] ); // @codingStandardsIgnoreLine Generic.PHP.NoSilencedErrors.Discouraged

		if ( null === $content ) {
			$content = $this->get_import_data( $selected_import );
		}

		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) { // WPCS: input var ok.
			wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Invalid content!', 'merlin-wp' ) ) );
		}

		$json			 = false;
		$this_content	 = $content[ $_POST['content'] ]; // @codingStandardsIgnoreLine WordPress.VIP.ValidatedSanitizedInput.InputNotValidated

		if ( isset( $_POST['proceed'] ) ) { // WPCS: input var ok.
			...
			$json = array(
				...
				'proceed'	 => 'true',
				'content'	 => $_POST['content'], // @codingStandardsIgnoreLine WordPress.VIP.ValidatedSanitizedInput.InputNotValidated
	...
	public function after_content_import_setup() {
		// Set static homepage.
		$homepage = get_page_by_title( apply_filters( 'merlin_content_home_page_title', 'Home' ) ); // @codingStandardsIgnoreLine WordPress.VIP.RestrictedFunctions.get_page_by_title
		...
		$blogpage = get_page_by_title( apply_filters( 'merlin_content_blog_page_title', 'Blog' ) ); // @codingStandardsIgnoreLine WordPress.VIP.RestrictedFunctions.get_page_by_title
	...
	public function before_content_import_setup() {
		...
		$hello_world = get_page_by_title( 'Hello World!', OBJECT, 'post' ); // @codingStandardsIgnoreLine WordPress.VIP.RestrictedFunctions.get_page_by_title
	...
	public function update_selected_import_data_info() {
		$selected_index = ! isset( $_POST['selected_index'] ) ? false : intval( $_POST['selected_index'] ); // WPCS: input var ok. CSRF ok.
			...
			<li class="merlin__drawer--import-content__list-item status status--Pending" data-content="<?php echo esc_attr( $slug ); ?>">
				<input type="checkbox" name="default_content[<?php echo esc_attr( $slug ); ?>]" class="checkbox" id="default_content_<?php echo esc_attr( $slug ); ?>" value="1" checked>
					<label for="default_content_<?php echo esc_attr( $slug ); ?>">
						...
						switch ( $slug ) {
							case 'options':
								esc_html_e( 'Colors & Style', '5th-avenue' );
								break;
							case 'redux':
								esc_html_e( 'Theme Options & Fonts', '5th-avenue' );
								break;
							case 'content':
							case 'widgets':
							case 'sliders':
